...
- `integrator_period` (double, default `0.1`): fixed integration period (seconds) used to advance odometry with the latest velocity sample.
...
## Timing and clocks
- Integration runs on a fixed-period timer (`integrator_period`) using the latest received velocity; arrival jitter does not change the step size.
