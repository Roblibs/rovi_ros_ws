...
class RoviOdomIntegratorNode : public rclcpp::Node
{
public:
  explicit RoviOdomIntegratorNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("rovi_odom_integrator", options),
    diag_updater_(this),
    steady_clock_(RCL_STEADY_TIME)
  {
    ...
    subscription_ = create_subscription<geometry_msgs::msg::Twist>(
      vel_topic_, vel_qos, std::bind(&RoviOdomIntegratorNode::handle_velocity, this, _1));
    ...
    if (diag_period_sec_ > 0.0) {
      diag_updater_.setHardwareID("rovi_odom_integrator");
      diag_updater_.setPeriod(diag_period_sec_);
      diag_updater_.add("timing", this, &RoviOdomIntegratorNode::produce_diagnostics);
    } else {
    ...
    integrator_timer_ = create_wall_timer(
      ...
      std::bind(&RoviOdomIntegratorNode::integrate_step, this));
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<RoviOdomIntegratorNode>());
  rclcpp::shutdown();
  return 0;
}
